...
- **`tour.js`** (paso 204) — el tour guiado de 9 pasos para cuentas nuevas: `TOUR_STEPS`, arranque/avance/salida (`checkTourAutoStart`/`enterTourStep`/`tourAdvance`/`tourFinish`/`tourSkipAll`/`tourRestart`) y el overlay de spotlight (`renderTourOverlay`, llamado al final de `render()`). Apunta directo a los `data-a`/`data-f` reales de cada vista (nunca un atributo `data-tour` aparte). La geometría del spotlight (`spotlightRectFor`/`spotlightMasksHtml`/`positionSpotlightCard`) está factorizada acá y la reusa `help-tutoriales.js` (paso 207) para no duplicarla.
- **`help-tutoriales.js`** (paso 207) — tutoriales de referencia por sección, repetibles cuando el usuario quiera (a diferencia del tour, que es de una sola pasada): `HELP_SECTIONS` (contenido real de las 9 secciones, guía rápida + guía completa con subtemas), `sectionHelpBtn()` (botón "¿Cómo se usa esto?" que suma `pageHead()` y la sub-pestaña Portal de Cuenta), `vTutorialesIndice()` (índice en Cuenta → Ayuda), `vTutPanel()` (el panel/modal) y el spotlight de "Mostrame" (`renderTutSpotOverlay`, mismo mecanismo que `tour.js`, llamado también al final de `render()`).
